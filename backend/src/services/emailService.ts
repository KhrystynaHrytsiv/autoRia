...
class EmailService {
    ...
    public async sendEmail(to: string, emailData: EmailDataType, context: Record<string, any>,): Promise<void> {
    ...
    private async _renderTemplate(templateName: string, context: Record<string, any>): Promise<string> {
        ...
        const childSource = await fs.readFile(path.join(process.cwd(), "src", "templates", `${templateName}.hbs`), "utf-8");
